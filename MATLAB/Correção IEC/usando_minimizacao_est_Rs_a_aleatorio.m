...
% load('curvas_sel.mat') %[2406, 2781, 146]
% curvas = curvas_selecionadas;

%load('IVCurves_IEC.mat')
...
Rs = 0;
erroRS = Inf;
minT = min([IVCurves(:).Tc]);
maxT = max([IVCurves(:).Tc]);
while( Rs > 0.42| Rs < 0.21 | erroRS > 0.005| a < 0)
    close all;
    %Tsel =  minT + rand*(maxT-minT);%+4
    Tsel = 55;
    Esel = 700;
    Tol = 1;
    id = NaN; k = 1;
    ...
    %% seleciona curvas aleatoriamente
    
    if (length(curvas) < qtdCurvas)
        idy = randperm(length(curvas), length(curvas));
    else
      idy = randperm(length(curvas), qtdCurvas);
    end
    curvas = curvas(idy);
    ...
    IVtransl = transladaCurvas(curvas, Tobj, Eobj, aIsc, bVoc, 0, 0, 0, 0);
    ...
    fprintf('erro_RS  = %.7f\n', erroRS);
    %%
    IVtransl = transladaCurvas(curvas, Tobj, Eobj, aIsc, bVoc, Rs, a, 0, 0);
end
%%
